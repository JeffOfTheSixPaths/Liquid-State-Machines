...
class LSM(nn.Module):
    def __init__(self, rec_mask=None,N_RES=N_RES, res_sparsity=0.8, in_sparsity=0.2):
        super().__init__()
        self.N_RES = N_RES
        ...
        #This is where it applies the mask
        with torch.no_grad():
    ...
    def forward(self, x):
        ...
        mem = torch.zeros((x.size(0), self.N_RES), device=x.device)
        spk = torch.zeros((x.size(0), self.N_RES), device=x.device)

        spike_sum = torch.zeros((x.size(0), self.N_RES), device=x.device)
...
if __name__ == '__main__':
    ...
    agents = []
    indicies = [ i for i in range(init_neurons) ]
    for machine in range(agent_num):
        mask = (torch.zeros(init_neurons, init_neurons)).float()

        for r, row in enumerate(mask):

            i = random.sample(indicies, intial_connections)

            for j in i:
                mask[r][j] = 1.0
        agents.append(LSM(rec_mask=mask, N_RES = init_neurons).to(device))
    ...
    def collect_states(loader, max_samples):
        ...
        for i, (data, target) in enumerate(loader):
            ...
            state = agents[0](x).detach().cpu().numpy()[0]
